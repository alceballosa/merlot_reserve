...
parser.add_argument(
    "-data_dir",
    dest="data_dir",
    default="/home/alberto/workspace/datasets/vcr",
    type=str,
...
out_fn = os.path.join(
    args.base_fn,
    "vcr",
    "{}{:03d}of{:03d}.tfrecord".format(args.split, args.fold, args.num_folds),
)
...
def draw_boxes_on_image(image, metadata, tokenl_to_names, flip_lr=False):
    ...
    font_i = ImageFont.truetype(
        font="/usr/share/fonts/truetype/ubuntu/DejaVuSans.ttf", size=17
    )
    ...
    for i in boxes_to_draw:
        ...
        color_i = tuple(
            (np.array(colorsys.hls_to_rgb(hue, lum, sat)) * 255.0)
            .astype(np.int32)
            .tolist()
        )
        txt_colori = tuple(
            (np.array(colorsys.hls_to_rgb(hue, txt_lum, sat)) * 255.0)
            .astype(np.int32)
            .tolist()
        )
...
def iterate_through_examples():
    ...
    with open(os.path.join(args.data_dir, args.split + ".jsonl"), "r") as f:
        for idx, l in enumerate(f):
            ...
            with open(
                os.path.join(args.data_dir, "vcr1images", item["metadata_fn"]), "r"
            ) as f:
                metadata = json.load(f)

            image = Image.open(
                os.path.join(args.data_dir, "vcr1images", item["img_fn"])
            )
            ...
            if "answer_label" in item:
                qar_query = "{} Answer: {}".format(
                    qa_query, qa_choices[item["answer_label"]]
                )
                ...
                if ("right" in everything) or ("left" in everything):
                    ...
                    img_lr = draw_boxes_on_image(
                        image, metadata, tokenl_to_names, flip_lr=True
                    )

                yield {
                    "qa_query": qa_query,
                    "qa_choices": qa_choices,
                    "qa_label": item["answer_label"],
                    "qar_query": qar_query,
                    "qar_choices": qar_choices,
                    "qar_label": item["rationale_label"],
                    "id": str(item["annot_id"]),
                    "image": img_boxes,
                    "image_fliplr": img_lr,
                }
            else:
                ...
                for i, qa_choice_i in enumerate(qa_choices):
                    ...
                    yield {
                        ...
                        "qar_choices": qar_choices,
                        "qar_label": 0,
                        "id": "{}-qar-conditioned_on_a{}".format(item["annot_id"], i),
                        "image": img_boxes,
                    }
...
with GCSTFRecordWriter(out_fn, auto_close=False) as tfrecord_writer:
    for j, ex in enumerate(iterate_through_examples()):
        feature_dict = {
            ...
            "image_fliplr": bytes_feature(
                pil_image_to_jpgstring(ex.get("image_fliplr", ex["image"]))
            ),
        ...
        for prefix in ["qa", "qar"]:
            ...
            if j < 20:
                ...
                print(
                    encoder.decode(
                        feature_dict[f"{prefix}_query"].int64_list.value,
